...
1. In the flow from the [tutorials](../getting-started/tutorials-r/) \([Episode 1](../getting-started/tutorials-r/season-1-the-local-experience/episode01.md)\), the `genre_movies` step calculates an artifact `movies`. We are going to demonstrate how this artifact can be inspected after the flow has executed;
