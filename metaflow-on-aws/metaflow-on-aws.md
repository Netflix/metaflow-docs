...
Datastore is a centralized data repository for all the data that's leveraged by and generated by Metaflow flows. In the local mode, all data artifacts are stored in a local directory. Metaflow integrates with Amazon S3 for cloud-scale storage so that you can process and persist larger amounts of data easily. Our [AWS CloudFormation template](deployment-guide/aws-cloudformation-deployment.md) creates a private bucket with appropriate permissions and if you would like to manually create such a bucket \(or use an existing one\), you can find the instructions [here](deployment-guide/manual-deployment.md#storage).

### Compute

Metaflow executes all steps in the flow as a separate local process in local mode. To run larger workloads which require resources that might not be available on a laptop \(think GPUs or 100s of GBs of RAM\), Metaflow integrates with AWS Batch to seamlessly run every step of the flow as a \(or many\) separate AWS Batch job\(s\). Our [AWS CloudFormation template](deployment-guide/aws-cloudformation-deployment.md) creates the necessary resources and roles and if you would like to manually create these resources \(or use existing resources\), you can find the instructions [here](deployment-guide/manual-deployment.md#compute). Note that, to execute your steps on AWS Batch, you would need to configure your [datastore](metaflow-on-aws.md#datastore) to use Amazon S3.
...
All necessary roles for accessing AWS Step Functions and Amazon EventBridge are automatically created by [our AWS CloudFormation template](deployment-guide/aws-cloudformation-deployment.md), but if you would like to set those up manually, please see [these instructions](deployment-guide/manual-deployment.md#scheduling).
