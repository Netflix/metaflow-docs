...
When running on AWS Step Functions, Metaflow code works exactly as it does locally: No changes are required in the code. All data artifacts produced by steps run on AWS Step Functions are available using the [Client API](../metaflow-r/client.md). All tasks are run on AWS Batch respecting the resources decorator, as explained in [Scaling Up and Out](../metaflow-r/scaling.md).
...
Let's use [the flow from the section about parameters](../metaflow-r/basics.md#how-to-define-parameters-for-flows) as an example:
...
In this case, the run should succeed without problems. If there were errors, you could reproduce them locally as explained in [Debugging with Metaflow](../metaflow-r/debugging.md#reproducing-production-issues-locally).
...
Note that step-functions create creates a new isolated [production namespace](../metaflow-r/tagging.md#production-namespaces) for your production flow. Please read [Organizing Results](../metaflow-r/tagging.md) to learn all about namespace behavior.
...
This option is similar to [`run --max-workers`](../metaflow-r/scaling.md#safeguard-flags) that is used to limit concurrency outside AWS Step Functions.
